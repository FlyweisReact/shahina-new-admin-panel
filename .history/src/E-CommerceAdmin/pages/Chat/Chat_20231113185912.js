...
const Chat = () => {
  ...
  useEffect(() => {
    const fetchDocumentData = async () => {
      const documentRef = doc(db, "yourCollectionName", "yourDocumentID");

      try {
        const documentSnapshot = await getDoc(documentRef);

        if (documentSnapshot.exists()) {
          // Extract data from the document and store in state
    ...
    // Call the function to fetch the specific document
    fetchDocumentData();
  }, []);
