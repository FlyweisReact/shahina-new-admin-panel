  ...
  const handleOnSubmit = async (e) => {
    ...
    if (db) {
      try {
        ...
        if (replyingTo) {
          // Ensure that replyingTo and its properties are defined
          if (replyingTo.userID && replyingTo.displayName) {
            ...
            return; // Abort adding the document if replyTo is invalid
          }
        }

        const docRef = await addDoc(collection(db, "messages"), messageData);

        // If replying to a message, update the existing document
        if (replyingTo) {
          await updateDoc(doc(db, "messages", docRef.id), {
            reply: arrayUnion(docRef.id), // Assuming reply is the field storing the replies
          });
        }
