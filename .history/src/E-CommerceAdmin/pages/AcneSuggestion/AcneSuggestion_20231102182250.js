  ...
  const deleteHandler = async (id) => {
    try {
      const { data } = await axios.delete(
        `${Baseurl}api/v1/admin/AcneQuizSuggession/deleteAcneQuizSuggession/${id}`,
        Auth
      );
    ...
    const putHandler = async (e) => {
      ...
      try {
        const { data } = await axios.put(
          `${Baseurl}api/v1/AcneQuizSuggession/updateAcneQuiz/${id}`,
          payload,
          Auth
        );
        toast.success(data.message);
        ...
        <Modal.Body>
          <Form onSubmit={edit ? putHandler : postHandler}>
            ...
            <Form.Group className="mb-3">
              ...
              <label for="html">Bundled Product</label>
              <input type="radio" id="css" name="fav_language" value="CSS" style={{marginRight : '10px'}} />
