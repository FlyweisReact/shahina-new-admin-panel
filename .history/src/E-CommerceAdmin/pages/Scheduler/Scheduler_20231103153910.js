...
const Scheduler = () => {
  ...
  console.log("18/10/2023".length)
  console.log(("18/10/2023").substr(4,6))
  //   2023, 10, 7, 23, 0
  const events = data?.map((item) => ({
    title: "Eyebrows waxing and threat",
    start: new Date(findYear(item.date) ,findMonth(item.date) , findDate(item.date)),
    end: new Date(findYear(item.date)), // Adjust this as needed for the end time
  }));
