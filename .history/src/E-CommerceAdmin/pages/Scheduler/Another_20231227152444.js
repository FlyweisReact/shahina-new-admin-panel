...
import { openModal, closeModal } from "../../../Store/Slices/modalSlices";
...
const Another = () => {
  ...
  const handleOpenEditDetailsModal = () => {
    dispatch(
      openModal({
        modalId: "editDetailsModal",
        showModal: true,
        modalData: { data }, // Pass your modal data here
      })
    );
  };

  const handleCloseEditDetailsModal = () => {
    dispatch(closeModal({ modalId: "editDetailsModal" }));
  };

  // Example: Access modal state and data
  const editDetailsModal = useSelector(selectModalById("editDetailsModal"));
