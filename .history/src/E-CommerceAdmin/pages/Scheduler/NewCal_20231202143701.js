...
const NewCal = () => {
  ...
  const blockedTimeRanges = [
    ...
    // Function to check if a given time slot is blocked
    const isTimeSlotBlocked = (date, startTime, endTime) => {
      ...
      for (const range of blockedTimeRanges) {
        const blockedStartTime = new Date(`${date} ${range.startTime}`);
        const blockedEndTime = new Date(`${date} ${range.endTime}`);
  
        if (
          currentDateTime >= blockedStartTime &&
          currentDateTime < blockedEndTime &&
          startDateTime < blockedEndTime &&
    ...
    // Function to customize the time scale cells
    const onTimeScaleCellRender = (args) => {
      if (isTimeSlotBlocked(args.date, args.startTime, args.endTime)) {
        args.element.style.backgroundColor = "lightgray"; // Customize the background color for blocked time slots
        args.element.style.pointerEvents = "none"; // Disable pointer events for blocked time slots
      }
    };
