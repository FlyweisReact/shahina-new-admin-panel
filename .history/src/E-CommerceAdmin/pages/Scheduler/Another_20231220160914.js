...
import { getBlockedSlots } from "../../../Respo/Api";
...
const Another = () => {
  ...
  const events = data?.flatMap((order) => {
    const firstName = order?.orders?.[0]?.user?.firstName
      ? order?.orders?.[0]?.user?.firstName
      : "";
    const lastName = order?.orders?.[0]?.user?.lastName
      ? order?.orders?.[0]?.user?.lastName
      : "";
    const fullName = firstName + " " + lastName;

    const originalDate = new Date(order?.orders?.[0]?.toTime);
    const modifiedDate = new Date(originalDate);
    modifiedDate.setHours(originalDate.getHours() - 5);
    modifiedDate.setMinutes(originalDate.getMinutes() - 30);

    
    const originalDate2 = new Date(order?.orders?.[0]?.toTime);
    const modifiedDate2 = new Date(originalDate);
    modifiedDate2.setHours(originalDate.getHours() - 5);
    modifiedDate2.setMinutes(originalDate.getMinutes() - 30);
    return {
      title: order?.orders?.[0]?.noShow ? (
        ...
        <div className="calender_slot">
          <p className="title">{fullName}</p>
          <div className="d-flex gap-2 " style={{ alignItems: "center" }}>
            {order?.orders?.[0]?.services?.[0]?.serviceId?.name}
            {order?.orders?.[0]?.orderStatus === "confirmed" ? (
      ...
      // start: new Date(order?.orders?.[0]?.toTime),
      // end: new Date(order?.orders?.[0]?.fromTime),
      start: modifiedDate,
      end: modifiedDate,
      id: order?.orders?.[0]?._id,
      isBlock: false,
      isShow: order?.orders?.[0]?.noShow,
    };
  });
  ...
  return (
    <>
      ...
      />
      <RescheduleCanvas show={openReschedule} handleClose={closeReschdule} />
