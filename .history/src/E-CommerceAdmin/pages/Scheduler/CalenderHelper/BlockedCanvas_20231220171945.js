...
const BlockedCanvas = ({ show, handleClose, startTime }) => {
  ...
  const monthFormated = start?.getMonth() + 1;
const dayFormated = start?.getDate();

// Ensure two-digit representation by adding leading zeros if needed
...
const formatedDate = `${year}-${monthStr}-${dayStr}`;
  const formatedDate = year + "-" + monthFormated + "-" + date;
  ...
  const submiHandler = (e) => {
    e.preventDefault();
    const payload = { title, description, date: formatedDate, from, to };
