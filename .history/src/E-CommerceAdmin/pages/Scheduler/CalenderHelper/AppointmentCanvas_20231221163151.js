...
export const AppointmentCanvas = ({ show, handleClose, startTime }) => {
  ...
  //     to_email: "Node4@flyweis.technology",
  ...
  const sendEmail = async (cart) => {
    ...
    // Iterate through the services in the cart
    for (const service of cart?.services) {
      ...
      if (pdfFileName) {
        // Generate dynamic URL for each PDF
        const pdfUrl = `http://localhost:3000/FormPdf/${pdfFileName}`;

        // Create the template parameters for email
        const templateParams = {
          to_email: "Node4@flyweis.technology",
          from_name: "React Flyweis",
          to_name: "Node",
          message: `PDF for ${serviceName}: ${pdfUrl}`,
        };

        // Add the attachment to the email
        attachments.push({
          filename: pdfFileName,
          content: pdfUrl, // You may need to fetch the actual PDF content here
        });
        ...
        try {
          ...
          console.log(`Email sent successfully for ${serviceName}`, response);
        } catch (error) {
