...
const AppointmentDetails = ({
  ...
  const Times = new Date(detail?.fromTime);
  const startingTime = Times?.toLocaleTimeString([], {
    hour: "2-digit",
    minute: "2-digit",
  });

  const ToTimes = new Date(detail?.toTime);
  const ending_time = ToTimes?.toLocaleTimeString([], {
    hour: "2-digit",
    minute: "2-digit",
  });
            ...
            {detail?.services?.map((i, index) => (
              ...
              >
                <div>
                  ...
                  <p className="faded">
                    {startingTime} - {ending_time} ({i.serviceId?.totalMin}min){" "}
                  </p>
