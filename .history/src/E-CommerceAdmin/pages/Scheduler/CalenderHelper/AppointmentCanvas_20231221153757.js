...
export const AppointmentCanvas = ({ show, handleClose, startTime }) => {
  ...
  const sendEmail = async () => {
    ...
    for (const service of find2.services) {
      ...
      if (pdfFileName) {
        // Assume your PDFs are in the public folder
        const servicePdfPath = `/FormPdf/${pdfFileName}`;
        const response = await fetch(servicePdfPath);
