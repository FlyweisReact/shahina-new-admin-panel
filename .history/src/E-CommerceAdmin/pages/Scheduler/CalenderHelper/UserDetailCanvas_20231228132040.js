...
import { closeModal, openModal, selectModalById } from "../../../../Store/Slices/modalSlices";
...
const UserDetailCanvas = ({ show, handleClose, setIsBooked, Details }) => {
  ...
  function useShow(id) {
    const { showModal } = useSelecto(selectModalById(id));
    return showModal;
  }
  ...
  return (
    <>
      <UserDialog
        show={useShow("UserDialog")}
        handleClose={() => closeThisOne("UserDialog")}
        data={Details}
      />
      <Modal
        show={modalShow}
