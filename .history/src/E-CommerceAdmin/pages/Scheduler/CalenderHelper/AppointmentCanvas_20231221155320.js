...
export const AppointmentCanvas = ({ show, handleClose, startTime }) => {
  ...
  //   service: "gmail",
  //   auth: { user: "info@shahinahoja.com", pass: "gganlypsemwqhwlh" },
  // });
  // let mailOptions = {
  ...
  const sendEmail = async () => {
    ...
    const attachments = [];
    const serviceToPdfPathMap = {
      // ... (your existing serviceToPdfPathMap)
    };
    ...
    for (const service of cart?.services) {
      ...
      const pdfFileName = serviceToPdfPathMap[serviceName];
      if (pdfFileName) {
        // Replace 'your_email_template_id' with your actual Email.js template ID
        ...
        const pdfFileContent = ""; // You need to get the actual PDF content here
