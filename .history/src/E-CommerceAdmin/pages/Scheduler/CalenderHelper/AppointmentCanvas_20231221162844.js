...
export const AppointmentCanvas = ({ show, handleClose, startTime }) => {
  ...
  const sendEmail = async () => {
    const templateParams = {
      to_email: "Node4@flyweis.technology", // Replace with the recipient's email address
      from_name: "React Flyweis",
      to_name: "Node",
      message: `http://localhost:3000/FormPdf/AQUAGOLD.pdf`,
    };

    const attachments = [
      {
        filename: "document.pdf", // Name of the attachment
        content: "base64_encoded_content", // Base64-encoded content of the PDF file
        encoding: "base64", // Encoding type
      },
    ];

    // Add attachments property to templateParams
    templateParams.attachments = attachments;
    console.log("templateParams:", templateParams);

    try {
      const response = await emailjs.send(
        "service_h5eugu8", // Replace with your Email.js service ID
        "template_ovrkmjp", // Replace with your Email.js template ID
        templateParams,
        "PhyItqgwaVGNR1kjF" // Replace with your Email.js user ID
      );
