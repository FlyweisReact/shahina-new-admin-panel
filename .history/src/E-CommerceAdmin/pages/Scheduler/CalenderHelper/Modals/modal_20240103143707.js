...
export const EditBookedService = ({
  ...
  const addInCart = async (e) => {
    e.preventDefault();
    await edit_service_in_order(orderId, payload, fetchCart);
    setShow(false);
  };
  ...
  const addAdOn = async (e) => {
    e.preventDefault();
    await edit_adonservice_in_order(orderId, payload, fetchCart);
    setShow(false);
  };
  ...
  useEffect(() => {
    if (totalTime) {
      ...
      const onlyHoursMatch = timeString.totalTime(/(\d+)\s*hr/);
      const onlyMinutesMatch = timeString.totalTime(/(\d+)\s*min/);
