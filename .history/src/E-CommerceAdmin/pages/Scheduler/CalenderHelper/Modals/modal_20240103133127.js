...
export const EditBookedService = ({
  ...
  const addInCart = async (e) => {
    e.preventDefault();
    await edit_service(orderId, payload, fetchCart);
    setShow(false);
  };
  ...
  useEffect(() => {
    if (totalTime) {
      const hoursAndMinutesMatch = totalTime.match(/(\d+)\s*hr\s*(\d*)\s*min/);
      const onlyMinutesMatch = totalTime.match(/(\d+)\s*min/);
      if (hoursAndMinutesMatch || onlyMinutesMatch) {
  ...
  useEffect(() => {
    if (type === "AdOn" && filteredService?.length > 0) {
      ...
      // Check if the value is different before setting
      if (filteredService?.[0]?.totalTime !== totalTime) {
        console.log("seeting")
        setTotalTime(filteredService?.[0]?.totalTime);
      }
    }
  }, [filteredService, serviceId, type, totalTime]);
