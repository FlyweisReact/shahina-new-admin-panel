...
import { deleteSuggestionOrder, editBookedNoted, getBookingDetail } from "../../../../Respo/Api";
...
const AppointmentDetails = ({
  ...
  function deleteSuggestion (suggesstionId) {
    const id = detail?._id
    deleteSuggestionOrder(id ,suggesstionId )
  }
        ...
        <>
          ...
          <div className="booked_service d-flex flex-wrap gap-4">
            {detail?.AddOnservicesSchema?.map((i, index) => (
              <div className="service_selector" key={`addOnservicesId${index}`}>
                <div>
          ...
          {detail?.suggesstion?.map((i, index) => (
            <div className="define_notes p-0" key={`notes${index}`}>
              ...
              <span className="d-flex gap-2">
                <i className="fa-regular fa-trash-can text-[#BF3131] cursor-pointer" />
                <i
