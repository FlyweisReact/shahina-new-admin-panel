...
const Another = () => {
  ...
  onNavigate={(newDate) => setSelectedDate(newDate)} // Update selectedDate on navigation
  components={{
    toolbar: (props) => (
      <>
        <CustomCalendarHeader {...props} />
        <div className="default-date-display">
          Default Date: {moment(selectedDate).format("YYYY-MM-DD")}
        </div>
      </>
    ),
  }}
