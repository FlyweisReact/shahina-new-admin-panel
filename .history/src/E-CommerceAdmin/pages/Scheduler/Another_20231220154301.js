...
const Another = () => {
  ...
  const events = data?.flatMap((order) =>
    order?.orders?.flatMap((item) => {
      const firstName = order?.orders?.[0]?.user?.firstName ? order?.orders?.[0]?.user?.firstName : "";
      const lastName = order?.orders?.[0]?.user?.lastName ? order?.orders?.[0]?.user?.lastName : "";
      ...
      return {
        title: item.noShow ? (
          ...
          <div className="calender_slot">
            <p className="title">{fullName}</p>
            <div className="d-flex gap-2 " style={{ alignItems: "center" }}>
              {item?.services?.[0]?.serviceId?.name}
