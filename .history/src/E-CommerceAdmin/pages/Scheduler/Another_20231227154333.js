...
const Another = () => {
  ...
  const handleShow = (modalId, data) => {
    // Deserialize the date string back to a date object
    const deserializedData = {
      ...data,
      slots: data.slots.map(slot => ({
        ...slot,
        startTime: new Date(slot.startTime),
        // Add similar conversion for other date fields if needed
      })),
    };
  
    openModalById(modalId, deserializedData);
  };
  ...
  // ---- Modal Slices
  const { showModal } = useSelector(selectModalById("appointmentCanvas"));
  ...
  const openModalById = (modalId, data) => {
    // Serialize the date object to string
    ...
    dispatch(openModal({ modalId, showModal: true, modalData: serializedData }));
  };
