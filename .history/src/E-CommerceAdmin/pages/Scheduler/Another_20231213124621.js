...
import ScrollableCalendar from "./CalenderHelper/ScrollableCalendar";
...
const Another = () => {
  ...
  const [selectedDate, setSelectedDate] = useState(new Date()); // State to
  ...
  const CustomCalendarHeader = ({ date }) => {
    ...
    const handleButtonClick = () => {
      ...
      // Check if the browser supports the 'showDatePicker' method
      if (datePicker.showDatePicker) {
        datePicker.showDatePicker();
      } else {
        // If not supported, focus on the input as a fallback
        datePicker.focus();
      }
    };
    return (
      <div className="date_selector" onClick={handleOpen}>
        <span> {value} </span>
        <i className="fa-solid fa-chevron-down"></i>

        <button onClick={handleButtonClick}>Open Date Picker</button>
        <input
          type="date"
          id="datePickerInput"
          value={selectedDate}
