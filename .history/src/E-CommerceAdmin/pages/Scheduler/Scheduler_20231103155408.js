...
const Scheduler = () => {
  ...
  function findMonth(date) {
    const number = String(data)?.slice(0, 2);
    return Number(number);
  }

  console.log(findMonth(17))

  //   2023, 10, 7, 23, 0
  const events = data?.map((item) => ({
    title: item.orders?.map((i) => i.services?.map((n) => n.serviceId?.name)),
    start: new Date(item.date?.slice(5), item.date?.slice(0, 2), 10, 23, 0),
    end: new Date(item.date?.slice(5), item.date?.slice(0, 2), 10, 23, 0),
  }));
