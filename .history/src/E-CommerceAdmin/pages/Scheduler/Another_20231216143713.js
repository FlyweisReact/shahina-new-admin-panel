...
const Another = () => {
  ...
  const events = data?.flatMap((order) =>
    order?.orders?.flatMap((item) =>
      item?.services?.map((service) => {
        let hour = parseInt(order.hr);
        const minute = parseInt(order.min);

        // console.log(item.fromTime);
        // console.log(item.toTime);

        const timestamp = "2023-12-30T05:30:00.000Z";
        const date = new Date(timestamp);

        const options = {
          timeZone: "GMT", // Adjust the timezone as needed
          hour12: false,
          hour: "2-digit",
          minute: "2-digit",
        };
        ...
        const start = new Date(item.fromTime);
        const start = new Date(item.toTime);
        ...
        return {
          ...
          id: item?._id,
          serviceId: service?.serviceId?._id,
          Time: `${item?.hr}:${item?.min} ${item?.timeFormat}`,
          FirstName: item?.user?.firstName,
          LastName: item?.user?.lastName,
          userEmail: item?.user?.email,
          userPhone: item?.user?.phone,
