...
const Cart = () => {
  ...
  function GiftModal(props) {
    ...
    const postHandler = async (e) => {
      ...
      try {
        const { data } = await axios.post(
          `${Baseurl}api/v1/admin/addtoCart/gift/${productId}`,
          payload,
          Auth
        );
        toast.success(data.message);
  ...
  function AdOnModal(props) {
    ...
    const postHandler = async (e) => {
      ...
      try {
        const { data } = await axios.post(
          `${Baseurl}api/v1/admin/addtoCart/addOnservices/${productId}`,
          payload,
          Auth
        );
        toast.success(data.message);
  ...
  function FrequentlyModal(props) {
    ...
    const getProducts = async () => {
      try {
        const res = await axios.get(`${Baseurl}api/v1/FrequentlyBuyProduct`);
        const data = res.data.data;
        setProducts(data);
      } catch {}
    };
    ...
    const postHandler = async (e) => {
      ...
      try {
        const { data } = await axios.post(
          `${Baseurl}api/v1/admin/addtoCart/frequentlyBuyProduct/${productId}`,
          payload,
          Auth
        );
        toast.success(data.message);
  ...
  const deleteGift = async (payload) => {
    try {
      const { res } = await axios.delete(
        `${Baseurl}api/admin/cart/delete/gift/${payload}/${id}`,
        Auth
      );
  ...
  const deleteProduct = async (payload) => {
    try {
      const { res } = await axios.delete(
        `${Baseurl}api/admin/cart/delete/product/${payload}/${id}`,
        Auth
      );
  ...
  const deleteService = async (payload) => {
    try {
      const { res } = await axios.delete(
        `${Baseurl}api/admin/cart/delete/service/${payload}/${id}`,
        Auth
      );
      toast.success("Removed");
      fetchData();
  ...
  const deleteFBP = async (payload) => {
    try {
      const { res } = await axios.delete(
        `${Baseurl}api/admin/cart/delete/frequentlyBuyProduct/${payload}/${id}`,
        Auth
      );
      toast.success("Removed");
      fetchData();
  ...
  const deleteAdOn = async (payload) => {
    try {
      const { res } = await axios.delete(
        `${Baseurl}api/admin/cart/delete/addOnservices/${payload}/${id}`,
        Auth
      );
      toast.success("Removed");
      fetchData();
  ...
  const checkoutHandler = async () => {
    try {
      const response = await axios.post(
        `${Baseurl}api/v1/admin/checkout/${id}`,
        {},
        Auth
      );
  ...
  const successOrder = async (id) => {
    try{
      const res = await axios.get(`https://shahina-backend.vercel.app/api/v1/admin/successOrder/${id}`)
  ...
  const cancelOrder = async (id) => {
    try{
      const res = await axios.get(`https://shahina-backend.vercel.app/api/v1/admin/cancelOrder/PXNEMVESZ/${id}`)
      toast.success("Order Created")
      fetchData()
    }catch{}
  }
      ...
      <section className="sectionCont">
        ...
        <div className="pb-4  w-full flex justify-between items-center">
          ...
          <div className="d-flex gap-2">
            ...
            {/* <button
              className="md:py-2 px-3 md:px-4 py-1 rounded-sm bg-[#042b26] text-white tracking-wider"
              onClick={() => setAdOnShow(true)}
            >
              Add Ad On Services
            </button> */}
            {/* <button
              className="md:py-2 px-3 md:px-4 py-1 rounded-sm bg-[#042b26] text-white tracking-wider"
              onClick={() => setFrequentShow(true)}
            >
              Add Frequently Product
            </button> */}
          </div>
        </div>
        ...
        <div className="overFlowCont">
          {data?.length === 0 || !data ? (
            ...
            <>
              {/* <p className="headP mt-5">Products</p>
              <Table>
                <thead>
                  <tr>
                    <th>Sno.</th>
                      ...
                      <td>
                        <span className="flexCont">
                          ...
                          <i
                            ...
                            onClick={() => deleteProduct(i.productId?._id)}
                          ></i>
                        </span>
                      </td>
                    </tr>
