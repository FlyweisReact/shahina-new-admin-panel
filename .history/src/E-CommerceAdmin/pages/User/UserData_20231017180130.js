  ...
  return (
    <>
      ...
      <section className="sectionCont">
          ...
          {ValueChecker(user?.subscriptionExpiration?.slice(0, 10), "Subscription Expiration")}
          {ValueChecker(user?.createdAt?.slice(0, 10), "Created At")}
   

    {user?.isSubscription === true ?     <div className="Desc-Container">
      <p className="title"> Subscription </p>
      <p className="desc"> Active </p>
    </div>  :     <div className="Desc-Container">
      <p className="title"> {string} </p>
      <p className="desc"> {holder} </p>
    </div>}
