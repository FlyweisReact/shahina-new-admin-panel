...
const Privacy = () => {
  ...
  const fetchData = async () => {
    try {
      const { data } = await axios.get(`${process.env.React_App_Baseurl}api/v1/static/getPrivacy`);
      setData(data.data);
  ...
  const deleteHandler = async (ide) => {
    try {
      const { data } = await axios.delete(`${process.env.React_App_Baseurl}api/v1/static/privacy/${ide}`, Auth);
      toast.success(data.message);
      fetchData();
    } catch (e) {
