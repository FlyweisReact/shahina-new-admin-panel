...
const Order = () => {
  ...
  const FinalFromDate =
    fromDate === '' || fromDate?.length < 5
      ? ''
      : `${fromDate}T00:00:00.000Z`;
  const FinalToDate =
    toDate === null || toDate?.length < 5 ? null : `${toDate}T23:59:59.000Z`;
  ...
  const getOrders = async () => {
    try {
      const response = await axios.get(
        `${Baseurl}api/v1/admin/ProductOrder?search=${search}&fromDate=${FinalFromDate}&toDate=${FinalToDate}`,
        Auth
      );
  ...
  useEffect(() => {
    getOrders();
    window.scrollTo(0, 0);
  }, [search, FinalFromDate, FinalToDate]);
