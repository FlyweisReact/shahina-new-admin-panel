  ...
  const getOrders = async () => {
    try {
      const response = await axios.get(
        `${Baseurl}api/v1/admin/ProductOrder?search=${search}&fromDate=${FinalFromDate}&toDate=${FinalToDate}&page=${page}&limit=${limit}`,
        Auth
      );
      ...
      <section>
        <section className="sectionCont">
          ...
          <div className="searchByDate">
            <div>
              <label>Showing : </label>
              <select onChange={(e) => setLimit} >
                <option> Showing {data?.length} out of {total} </option>
