  ...
  const fetchData = async () => {
    try {
      const { data } = await axios.get(
        `${Baseurl}api/v1/Product/all/paginateProductSearch?page=${page}&limit=${limit}&search=${query}`
      );
      setData(data.data);
  ...
  const deleteHandler = async (id) => {
    try {
      const { data } = await axios.delete(
        `https://krish-vapes-backend.vercel.app/api/v1/Product/deleteProduct/64edf88dcc0ee17a1763a502`,
        Auth
      );
