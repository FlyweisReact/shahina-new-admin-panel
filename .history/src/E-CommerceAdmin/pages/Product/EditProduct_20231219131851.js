...
const EditProduct = () => {
  ...
  function formHandler(key, cmd) {
    if (key === null || key  === null undefined || key?.length != 0) {
      return cmd;
    }
  }
  ...
  formHandler(nutritionId, fd.append("nutritionId", nutritionId));
  formHandler(skinTypeId, fd.append("skinTypeId", skinTypeId));
  formHandler(productTypeId, fd.append("productTypeId", productTypeId));
  formHandler(brandId, fd.append("brandId", brandId));
  formHandler(skinConditionId, fd.append("skinConditionId", skinConditionId));
  fd.append("name", name);
