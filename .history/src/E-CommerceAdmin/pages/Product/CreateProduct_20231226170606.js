...
const CreateProduct = () => {
  ...
  const pushAllIds = (e, arr, setData) => {
    console.log(e.target.checked, arr, setData);

    const allIds = nutritionArray.map((item) => item._id);
    setNutritionId(allIds);
    console.log(allIds);
  };
    ...
    <section>
      <section className="sectionCont">
        ...
        <Form onSubmit={createProduct}>
          ...
          <Button variant="dark">Push All Nutrition IDs</Button>
          ...
          <Form.Group className="mb-3">
            ...
            <Form.Check
              type="checkbox"
              label={`All`}
              onClick={(e) => pushAllIds(e.target.checked , nutritionArray, setNutritionId)}
            />
