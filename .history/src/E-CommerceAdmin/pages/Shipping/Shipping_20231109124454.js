  ...
  function MyVerticallyCenteredModal(props) {
    ...
    return (
      ...
      >
        ...
        {ValueChecker(edit?.sender?.address?.address_line1, "Sender Address Line 1")}
        ...
        {ValueChecker(edit?.receiver?.address?.address_line1, "Reciever Address Line 1")}
        ...
        {ValueChecker(edit?.scheduling?.estimated_delivery_date_minimum , "Estimated delivery date minimum")}
        {ValueChecker(edit?.scheduling?.estimated_delivery_date_maximum , "Estimated delivery date maximum")}
        {ValueChecker(edit?.scheduling?.status , "Scheduling status")}
        {ValueChecker(edit?.scheduling?.is_cancellable , "Schedulling is Cancelled")}

        {ValueChecker(edit?.volume?.value, "Volume Value")}
        {ValueChecker(edit?.volume?.units, "Volume Unit")}

        {ValueChecker(edit?.route?.delivery_guarantee_status, "Route Delivery Gaurantee Status")}
        ...
        {ValueChecker(edit?.price_breakdown?.base?.amount, "Price breakdown base amount")}
        {ValueChecker(edit?.price_breakdown?.base?.currency, "Price breakdown base currency")}
        {ValueChecker(edit?.price_breakdown?.base_tax?.amount, "Price breakdown base tax amount")}
        {ValueChecker(edit?.price_breakdown?.base_tax?.currency, "Price breakdown  currency")}
